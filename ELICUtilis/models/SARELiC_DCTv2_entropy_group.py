...
class SAREliC(CompressionModel):
    ...
    def forward(self, x, gp, mode_quant="ste"):
        y = self.g_a(x)
        z = self.h_a(y)
        ...
        anchor_split = tuple(anchor[:, indices, :, :] for indices in gp.values())#torch.split(anchor, self.groups[1:], dim=1)
        non_anchor_split = tuple(non_anchor[:, indices, :, :] for indices in gp.values())#torch.split(non_anchor, self.groups[1:], dim=1)

        y_slices = tuple(y[:, indices, :, :] for indices in gp.values())#torch.split(y, self.groups[1:], dim=1)
