...
class SAREliC(CompressionModel):
    ...
    def compress(self, x, return_intermediates=False):
        ...
        rings = self.entropy_bottleneck.compress(z)
        z_hat = self.entropy_bottleneck.decompress(z_strings, z.size()[-2:])
        ...
        if return_intermediates:
            result.update({
                "y": y,  # Latent representation from g_a
                "z": z,  # Hyperprior latent from h_a
                "z_hat": z_hat,  # Quantized hyperprior latent
                "latent_means": latent_means,  # Hyperprior means
                "latent_scales": latent_scales,  # Hyperprior scales
                # "y_hat_slices": y_hat_slices,  # Quantized latent slices
                # "support_list": support_list,  # Context information for each slice
                # "y_slices": y_slices,  # Original latent slices
            })
