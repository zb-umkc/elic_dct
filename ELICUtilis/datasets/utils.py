...
class SarIQDataset(Dataset):
    def __init__(self, root, train=True, names=False, min_val=-5000, max_val=5000, n_bits=12) -> None:
        super().__init__()
        self.train = train
        self.names = names # return name of image as well

        self.n_values = 2**n_bits - 1
        self.min_val = min_val 
        self.max_val = max_val
        # self.amp_max_val = np.sqrt(2*max_val**2)
        ...
        # self.amp_pol1 = os.path.join(root, "amp")
        # self.amp_pol2 = os.path.join(root, "amp_VV")
        # self.amp_pol3 = os.path.join(root, "amp_HV")
        # self.amp_pol4 = os.path.join(root, "amp_VH")
    ...
    def _train_transforms(self, gt_image_pol):  #, amp_image_pol1 , amp_image_pol2, amp_image_pol3, 1, gt_image_pol2, gt_image_pol3, gt_image_pol4, amp_image_pol4
        """
        Random image transforms.
        """
        if random.random() > 0.5:
            gt_image_pol = TF.hflip(gt_image_pol)
            # amp_image_pol1 = TF.hflip(amp_image_pol1)
            # gt_image_pol2 = TF.hflip(gt_image_pol2)
            # amp_image_pol2 = TF.hflip(amp_image_pol2)
            # gt_image_pol3 = TF.hflip(gt_image_pol3)
            # amp_image_pol3 = TF.hflip(amp_image_pol3)
            # gt_image_pol4 = TF.hflip(gt_image_pol4)
            # amp_image_pol4 = TF.hflip(amp_image_pol4)
        if random.random() > 0.5:
            gt_image_pol = TF.vflip(gt_image_pol)
            # amp_image_pol1 = TF.vflip(amp_image_pol1)
            # gt_image_pol2 = TF.vflip(gt_image_pol2)
            # amp_image_pol2 = TF.vflip(amp_image_pol2)
            # gt_image_pol3 = TF.vflip(gt_image_pol3)
            # amp_image_pol3 = TF.vflip(amp_image_pol3)
            # gt_image_pol4 = TF.vflip(gt_image_pol4)
            # amp_image_pol4 = TF.vflip(amp_image_pol4)
        if random.random() > 0.5:
            random_rotate = random.choice([-90, 90])
            gt_image_pol = TF.rotate(gt_image_pol, random_rotate)
            # amp_image_pol1 = TF.rotate(amp_image_pol1, random_rotate)
            # gt_image_pol2 = TF.rotate(gt_image_pol2, random_rotate)
            # amp_image_pol2 = TF.rotate(amp_image_pol2, random_rotate)
            # gt_image_pol3 = TF.rotate(gt_image_pol3, random_rotate)
            # amp_image_pol3 = TF.rotate(amp_image_pol3, random_rotate)
            # gt_image_pol4 = TF.rotate(gt_image_pol4, random_rotate)
            # amp_image_pol4 = TF.rotate(amp_image_pol4, random_rotate)
            
        
        return gt_image_pol  #, amp_image_pol1, 1, gt_image_pol2, gt_image_pol3, gt_image_pol4
    # , amp_image_pol2, gt_image_pol3, amp_image_pol3, gt_image_pol4, amp_image_pol4
